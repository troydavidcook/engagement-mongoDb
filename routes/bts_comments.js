...
router.post('/', middleware.isLoggedIn, (req, res) => {
  const imageId = req.params.id;
  BtsImage.findById(imageId, (err, image) => {
    if (err) {
      console.log('Error: ', err);
      res.redirect('/bts_images');
    } else {
      ...
      Comment.create(req.body.comment, (error, comment) => {
        if (err) {
          ...
          res.redirect(`/bts_images/${imageId}`);
        }
      });
    }
  });
...
router.get('/:comment_id/edit', middleware.checkCommentOwnership, (req, res) => {
  ...
  Comment.findById(commentId, (err, fetchedComment) => {
    if (err) {
      ...
      res.render('comments/bts_edit', { bts_image_id: req.params.id, comment: fetchedComment });
      // Comment edit route
    }
  });
});
...
router.put('/:comment_id', middleware.checkCommentOwnership, (req, res) => {
  ...
  Comment.findByIdAndUpdate(commentId, req.body.comment, (err, updatedComment) => {
    if (err) {
      ...
      res.redirect(`/bts_images/${btsImageId}`);
    }
  });
});
